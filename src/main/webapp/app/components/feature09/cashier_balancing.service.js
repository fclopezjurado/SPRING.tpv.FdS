angular.module("tpv").service("balancesService", ['$http', '$q', function($http, $q) {
	"use strict";
	...
	const baseURL = "http://localhost:8080/SPRING.tpv.FdS.1.2.0-SNAPSHOT/api/v0";
	const balancesURL = "/balances";
	const ticketsURL = "/tickets";

	
	this.findAll = function() {						
		return this.request({ method: 'GET', url: baseURL + balancesURL });
	}
	...
	this.getTotal = function(date) {
		return this.request({
			method: 'GET',
			url: baseURL + ticketsURL + balancesURL + "?day="+date,			
		});
	}
